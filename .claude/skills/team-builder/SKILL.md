...
|---|---------|--------|---------------------|------|--------|------|
| 1 | {名前} | {type1}/{type2} | {H/A/B/C/D/S} | {特性} | {持ち物} | 軸 |
| 2 | ... | ... | ... | ... | ... | 攻め補完 |
| ... |
...
|-----------|------------|------------|-----|-----------|
（18タイプすべてについて、各メンバーの**特性補正込み**倍率を表示）
（免疫特性による無効は `0` で表示。耐性変化特性による修正倍率で表示）
...
**出力先**: `{カレントディレクトリ}/{軸ポケモン名}-build-{YYYY-MM-DD}.txt`
...
- `./pokemons/<name>/` 配下にbreedスキルの出力ファイルが存在する場合、そこから実数値・努力値・性格を読み取る
